...
void DetectLane::detectLeftRight(const vector<vector<Point> > &points, Point& mass_road)
{
    ...
    vector<int> lane1_cal = recognize_left_right( lane1, mass_road );
    vector<int> lane2_cal = recognize_left_right( lane2, mass_road );
    if (  lane1_cal.at(0) == LEFT_LANE )
    {
        for (int i = 0; i < lane1.size(); i++)
        {
            leftLane[floor(lane1[i].y / slideThickness)] = lane1[i];
        }
        var_left = lane1_cal.at(1);
    }
    else if( lane1_cal.at(0) == RIGHT_LANE)
    {
        for (int i = 0; i < lane1.size(); i++)
        {
            rightLane[floor(lane1[i].y / slideThickness)] = lane1[i];
        } 
        var_right = lane1_cal.at(1); 
    }
    else 
    {
        var_left = 0;
...
vector<int> DetectLane::recognize_left_right( vector<Point>& lanex , Point& mass_road )
{
    ...
    // true is left lane
    vector<int> para ;
    para.push_back( ( x_line <= x_mass )? LEFT_LANE : RIGHT_LANE );
    para.push_back( (int)variance );
    return para;
}
...
Point DetectLane::MassOfRoad(const Mat &src_RGB)
{
    ...
    findContours( road_thresh_inv, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0) );
